...
# Optimized for use with version 3.3.3 of the Wrapper.
...
if [ "$FIRST_CHAR" != "/" ]
then
    PIDDIR= $REALDIR_OLD /$PIDDIR
fi
# Same test for WRAPPER_CMD
FIRST_CHAR=`echo $WRAPPER_CMD_OLD | cut -c1,1`
if [ "$FIRST_CHAR" != "/" ]
then
    WRAPPER_CMD= $REALDIR_OLD /$WRAPPER_CMD
fi
# Same test for WRAPPER_CONF
FIRST_CHAR=`echo $WRAPPER_CONF | cut -c1,1`
if [ "$FIRST_CHAR" != "/" ]
then
    WRAPPER_CONF= $REALDIR_OLD /$WRAPPER_CONF
fi
