...
/**
 */
public class GlobalVariableQuickfixTest extends BashCodeInsightFixtureTestCase {
    ...
    public void testUnregisterGlobalVar() throws Exception {
        ...
        List<IntentionAction> actions = myFixture.filterAvailableIntentions("Register as global variable");
        ...
        Assert.assertTrue("The variable must be registered now", BashProjectSettings.storedSettings(getProject()).getGlobalVariables().contains("GLOBAL_VAR"));
        Assert.assertEquals("The register action must be unavailable now", 0, myFixture.filterAvailableIntentions("Register as global variable").size());

        Assert.assertEquals("The unregister action must be available now", 1, myFixture.filterAvailableIntentions("Unregister as global variable").size());
        ...
        Assert.assertEquals("The register action must be available again", 1, myFixture.filterAvailableIntentions("Register as global variable").size());
        Assert.assertEquals("The unregister action must be unavailable", 0, myFixture.filterAvailableIntentions("Unregister as global variable").size());
    }

    @Override
    protected String getBasePath() {
        return "/quickfixes/registerGlobalVarQuickfix/";
    }
}