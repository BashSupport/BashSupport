...
public class BashSimpleTextPreprocessorTest extends AbstractTextPreprocessorTest {
    ...
    public void testPatching() throws Exception {
        String original = "  abc  \\$a=\\$b=\\$c\\$d$e";
        TextPreprocessor preprocessor = new BashSimpleTextPreprocessor(TextRange.allOf(original));
        StringBuilder out = new StringBuilder();
        preprocessor.decode(original, out);

        Assert.assertEquals("  abc  $a=$b=$c$d$e", out.toString());

        Assert.assertEquals("  abc   $a= $b= $c $d$e", preprocessor.patchOriginal(original));
    }
