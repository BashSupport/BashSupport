...
public final class BashResolveUtil {
    ...
    public static PsiElement resolve(BashVarImpl bashVar, boolean leaveInjectionHosts) {
        ...
        VirtualFile virtualFile = bashVar.getContainingFile().getVirtualFile();
        ...
        Collection<BashIncludeCommand> includeCommands = canonicalPath != null
                ? StubIndex.getElements(BashIncludeCommandIndex.KEY, canonicalPath, project, fileScope, BashIncludeCommand.class)
        ...
        if (!includeCommands.isEmpty()) {
            ...
            for (BashIncludeCommand command : includeCommands) {
                ...
                if (varIsInFunction || includeIsInFunction || (bashVar.getTextOffset() > command.getTextRange().getEndOffset())) {
